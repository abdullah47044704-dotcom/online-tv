...
let favorites = [];
try {
  const saved = localStorage.getItem("abdullah_tv_favorites");
  if (saved) {
...
function setupVolumeControl() {
  const video = document.getElementById("video");
  if (!video) return;
  const isMutedSaved = localStorage.getItem("abdullah_tv_muted") === "true";
...
function toggleMute() {
  ...
  localStorage.setItem("abdullah_tv_muted", video.muted);
}
...
function toggleFavorite(event, url) {
  ...
  localStorage.setItem("abdullah_tv_favorites", JSON.stringify(favorites));
...
function playChannel(index) {
  ...
  if (typeof Hls !== "undefined" && Hls.isSupported()) {
    currentHls = new Hls({
      enableWorker: true,
      lowLatencyMode: false,            // Disabled LL-HLS to prioritize smooth playback on slow nets
      liveSyncDurationCount: 6,         // Stay further behind live edge to build a strong buffer
      liveMaxLatencyDurationCount: 12,  // Allow more latency flexibility to avoid buffering
      maxBufferLength: 60,              // Store up to 60 seconds of video in advance
      maxMaxBufferLength: 90,           // Absolute max buffer length
      backBufferLength: 30              // Keep previous segments in memory for smooth seeks
    });
    currentHls.loadSource(channel.url);
    currentHls.attachMedia(video);

    currentHls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
      // Force highest available quality/resolution
      if (data && data.levels && data.levels.length > 0) {
        currentHls.currentLevel = data.levels.length - 1; // Highest index is usually highest quality
      }

      video.muted = false;
      if (typeof updateVolumeButtonState === "function") {
...
function showTermsModal(type, event) {
  ...
  const text = type === "privacy" 
    ? "At ABDULLAH TV 📺, we value your privacy. We do not collect or store any personal data. All streams are sourced from third-party public playlists and played locally in your browser or application. Your preferences are saved only on your local device."
    : "Welcome to ABDULLAH TV 📺! Our services are provided free of charge for streaming live channels. We do not host any of the video content; all streams are sourced from publicly available public playlists. By using this app, you agree to comply with your local copyright and streaming laws.";
...
function toggleTheme() {
  const checkbox = document.getElementById("checkboxTheme");
  if (!checkbox) return;
...
function setupMobileAppBanner() {
  ...
  const isBannerHidden = localStorage.getItem("abdullah_tv_hide_app_banner") === "true";
...
function closeAppBanner() {
  ...
  if (banner) {
    ...
    localStorage.setItem("abdullah_tv_hide_app_banner", "true");
  }
}
  ...
  fetch(configUrl)
    ...
    .then(data => {
      if (data && data.buildCode && data.buildCode > currentBuildCode) {
        ...
        const laterTime = localStorage.getItem("abdullah_tv_update_later_time");
        const laterBuild = localStorage.getItem("abdullah_tv_update_later_build");
...
function closeUpdateModal() {
  ...
  if (modal) {
    ...
    if (updateBuild) {
      localStorage.setItem("abdullah_tv_update_later_time", Date.now().toString());
      localStorage.setItem("abdullah_tv_update_later_build", updateBuild);
    }
  }
}
...
function checkDisclaimer() {
  const accepted = localStorage.getItem("abdullah_tv_disclaimer_accepted");
  if (!accepted) {
...
function acceptDisclaimer() {
  localStorage.setItem("abdullah_tv_disclaimer_accepted", "true");
